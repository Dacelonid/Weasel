...
public class Weasel {
    ...
    private void start() {
        ...
        checkAllCharactersAreValid(WEASEL);
        ...
        do {
            ...
            iteration++;
        } while (!WEASEL.equals(nextSuggestion));
        System.out.printf("Solution found after %d iterations%n", iteration);
    }

    private static String getNextSuggestion(final String lastSuggestion) {
        String suggestion = "";
        int score = 0;
        for (int x = 0; x < 100; x++) {
            final String nextResult = getRandomString(lastSuggestion, WEASEL);
    ...
    public static String getRandomString(final String lastSuggestion, final String wantedPhrase) {
    ...
    public static Set<Integer> getScore(final String suggestion) {
        final Set<Integer> correctLetters = new HashSet<Integer>();
        if (suggestion.equals("")) {
            return correctLetters;
        }
        for (int x = 0; x < WEASEL.length(); x++) {
            if (WEASEL.charAt(x) == suggestion.charAt(x)) {
